...
- **Historial clínico**: registro de pacientes (DNI, edad, sexo, estatura,
  tabaquismo) persistido como JSON en `Historiales_Medicos/`.
...
patients.py         Persistencia del historial clínico (JSON por paciente)
...
- `Historiales_Medicos/`, `logs/` y `venv/` están excluidos del control de
  versiones (ver `.gitignore`); son datos/artefactos locales.
